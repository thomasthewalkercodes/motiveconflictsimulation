...
for run_dir in resolve_run_dirs(RUNS_TO_ANALYZE):
    ...
    # One mean + one variance of the jump distribution per CSV, kept grouped
    # by the agent that produced the CSV so the summary can split them out.
    by_agent = {}  # agent -> {"means": [...], "variances": [...]}
    for sim_file in sim_files:
        ...
        bucket = by_agent.setdefault(agent, {"means": [], "variances": []})
    ...
    for agent in sorted(by_agent):
        ...
        std_deg = mean_var ** 0.5

        print(
            f"  agent {agent}: {len(means)} csv | "
            f"mean {mean_jump:.3f} deg | std {std_deg:.3f} deg"
        )
        ...
        summary_rows.append(
            {
                ...
                "mean_variance_deg2": round(mean_var, 4),
